...
export const ADD_TODO = async (data: { name: string; description: string }) => {
  try {
...
export const UPDATE_TODO = async (todo:{id:number,name:string,description:string}) => {
    try {
      const allTodos = await axios.put(`http://localhost:3001/todos/${todo.id}`,todo);
      return allTodos.data;
    } catch (error) {
      console.error(error);
    }
